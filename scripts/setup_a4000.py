...
def stage_llama_server(user: str) -> None:
    ...
    write_file(
        ...
        textwrap.dedent(f"""\
            ...
            Exec=--model /models/llm/qwen2.5-14b-instruct-q4_k_m-00001-of-00003.gguf \\
                --n-gpu-layers 999 \\
                --flash-attn \\
                --ctx-size 8192 \\
                --threads 8 \\
...
def stage_llm_swap_helper(user: str) -> None:
    write_file(
        ...
        textwrap.dedent(f"""\
            ...
            #
            # Uses stop + podman rm + start (instead of restart) because
            # the CUDA container image can reuse a stale container on
            # restart, ignoring the updated Quadlet parameters.
            #
            # Examples:
            #   llm-swap /mnt/data/models/llm/qwen2.5-7b-instruct-q4_k_m-00001-of-00002.gguf "qwen2.5-7b-instruct" 8192 --slots 4
            ...
            CONTAINER_NAME="llama-server-llm"
