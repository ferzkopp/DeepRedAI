#!/usr/bin/env bash
# Single driver for the Phase 3 p3-v3 run: generate, audit, build, train, evaluate.
# p3-v3 changes one thing against p3-v2: `retain` is regenerated at salience.
set -Eeuo pipefail

ROOT=/mnt/data/DeepRedAI
LABEL=${LABEL:-p3-v3}
MODEL_TAG=${MODEL_TAG:-p3v3}
...
# p3-v3 inherits every asset from p3-v2 except `retain`, which is regenerated.
seed_corpus() {
  require_dir "$CORPUS"
  require_file "$SYSTEM_PROMPTS"
  for kind in retain_formats era_native era_native_formats \
              persona persona_identity; do
    require_file "$CORPUS/$kind/$kind.jsonl"
  done
}

TARGET_RETAIN=${TARGET_RETAIN:-6000}

stage_generate() {
  seed_corpus
  require_endpoint "$FACT_ENDPOINT" 'salient retain'
  local file="$CORPUS/retain/retain.jsonl"
  local have=0
  [[ -s "$file" ]] && have=$(wc -l < "$file")
  local remaining=$(( TARGET_RETAIN - have ))
  if (( remaining <= 0 )); then
    echo "retain already complete: $have/$TARGET_RETAIN"
    return 0
  fi
  printf '\n== Regenerate retain at salience: %d of %d remaining ==\n' \
    "$remaining" "$TARGET_RETAIN" | tee -a "$CORPUS/generation.log"
  python3 scripts/generate_deepred_corpus.py \
    --kind retain --target "$remaining" \
    --per-article 5 --batch-articles 12 \
    --output-dir "$CORPUS" --endpoint "$FACT_ENDPOINT" \
