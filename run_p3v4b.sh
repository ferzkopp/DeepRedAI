#!/usr/bin/env bash
# Phase 3 p3-v4b: p3-v4's persona stage on a corpus that is actually p3-v2.
#
# p3-v4 failed every gate, but not because the persona stage failed: its dataset
# omitted era_native_formats and retain_formats entirely and capped era_native
# and retain at 1500, cutting the corpus from 20,116 rows to 9,667 and
# era-native content by 77%. That confounded the run. This driver restores the
# full p3-v2 mix and adds deterministic marker injection, which is the actual
# persona lever: p3-v4 paired the system prompt with marker-free answers 2.1:1,
# so the model learned to drop the voice on factual questions.
set -Eeuo pipefail

ROOT=/mnt/data/DeepRedAI
LABEL=${LABEL:-p3-v4b}
MODEL_TAG=${MODEL_TAG:-p3v4b}
...
# Reuse p3-v4's corpus: the 2,216 restyled persona_capability rows cost 243 GPU
# minutes and nothing about them was wrong.
CORPUS=${CORPUS:-/mnt/data/deepred_corpus/p3-v4}
...
# Marker injection rates. The controls stay at zero: they carry no system prompt
# and are the contrast that keeps the voice conditional rather than constant.
...
for pct in ('010', '025', '050', '075', '100'):
    ...
    # `x or default` turns a legitimate 0.0 into the worst case, so a perfect
    # score failed its own gate. Missing metrics still fail closed.
    def worst(value, default):
        return default if value is None else value
    checks = {
        'persona': worst(m['persona'], 0.0) >= 0.50,
        'era_native_kept': worst(m['era_native'], 0.0) >= BACKBONE_ERA - 0.05,
        'leak_kept': worst(m['conversational_modern_leak'], 1.0)
                     <= BACKBONE_LEAK + 0.05,
        'pre_1969_vs_base': worst(m['pre_1969_recall'], 0.0)
                            >= worst(metrics[base_id]['pre_1969_recall'], 0.0),
        'utility_vs_base': ratio >= 1.0,
        'repetition': worst(m['repetition_or_boilerplate'], 1.0) <= 0.05,
    }
